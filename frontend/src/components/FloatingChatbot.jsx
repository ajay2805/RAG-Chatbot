...
import { HiOutlineSparkles, HiOutlineChatAlt2, HiOutlineMicrophone, HiOutlineRefresh } from 'react-icons/hi';
  ...
  useEffect(() => {
    ...
    const loadLauncherAnimation = async () => {
      try {
        ...
        // Check if it's a valid Lottie JSON (must have layers array)
        if (isMounted && animationJson && Array.isArray(animationJson.layers)) {
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          ...
          Swal.fire({ title: 'Error', icon: 'error', text: 'Could not process voice input.' });
        } finally {
          setIsSending(false);
        }
      };
      ...
      Swal.fire({ title: 'Error', icon: 'error', text: 'Microphone access denied.' });
    }
  };
    ...
    >
      ...
      >
          ...
          <div className="floating-chatbot-footer">
            <div className="floating-chatbot-input-group">
              <input
                type="text"
                value={message}
